...
import com.example.presence.data.Prediction
            ...
            for (face in faces) {
                try {
                    ...
                    if (maskLabel == maskDetectionModel.NO_MASK) {
                        ...
                        {
                            val databaseReference=FirebaseDatabase.getInstance("https://presence-23cbb-default-rtdb.asia-southeast1.firebasedatabase.app").getReference("Users")
                            databaseReference.child("${FirebaseAuth.getInstance().currentUser?.uid}").get().addOnSuccessListener {
                                if(bestScoreUserName==it.child("fullName").value)
                                {
